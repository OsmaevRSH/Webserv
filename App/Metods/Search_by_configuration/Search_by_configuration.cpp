...
t_server Search_by_configuration::get_server()
{
	...
	for (; it < _config._servers.end(); ++it)
	{
		...
		{
			if ("127.0.0.1:" + std::to_string(it->port) == _handler.get_server_ip() + port ||
				"localhost:" + std::to_string(it->port) == _handler.get_server_ip() + port)
				return *it;
		}
		if (it->ip + ":" + std::to_string(it->port) == _handler.get_server_ip() + port)
			return *it;
	}

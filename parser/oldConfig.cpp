...
/*   Updated: 2020/12/22 17:16:39 by jeldora          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Config::		parse(std::string text)
{
	...
	while (next_word_pos(text, pos))
	{
		...
		{
			_Server new_server;
			new_server.parse(dir_content, pos);
			_servers.push_back(new_server);
		}
		else
			select_directive(word, dir_content, _ew, pos);
		pos = end_pos + 1;
	}
}

void	Config::_Server::parse(std::string text, const size_t base_pos)
{
	size_t pos = 0;
	...
	while (next_word_pos(text, pos))
	{
		...
		{
			_Route new_route;
			new_route.parse(dir_content, base_pos + pos);
			_routes.push_back(new_route);
		}
		else
			select_directive(word, dir_content, _ew, *this, base_pos + pos);
		pos = end_pos + 1;
	}
}

void	Config::_Route::parse(std::string text, const size_t base_pos)
{
	size_t pos = 0;
